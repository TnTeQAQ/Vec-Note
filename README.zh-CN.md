...
- **前端：** React 19 + Vite + TypeScript（向量化、签名、验签全部在浏览器完成）
- **后端：** Cloudflare Workers（静态托管 + `/api`）
- **数据库：** Cloudflare D1（SQLite）
- **加密：** [`@noble/curves`](https://github.com/paulmillr/noble-curves) BLS12-381

## 功能特性

- 发布标题 + Markdown 内容的留言，只上传内容、公开向量与签名
- 基于密封向量余弦相似度的模糊搜索（30% 阈值 + 至少共享一个 n-gram 槽位）
- 手动核查弹窗：输入候选标题与密文做 BLS 验签——应用不做自动猜测
- 隐藏管理入口（长按）：删除留言、置顶、修改密码
- 发布限流、管理会话 token、PBKDF2-SHA256 密码哈希
- 明暗主题跟随系统、平滑滚动、弹窗开关动画
- 一条命令本地开发，本地 D1 开箱即用——体验项目不需要 Cloudflare 账号
...
4. **置顶**：管理模式下可对任意留言置顶 / 取消置顶（`PATCH /api/notes/:id/pin`，需 Bearer 会话）。置顶留言对**所有访客**显示「置顶」徽标，并永远排在「最新留言」与搜索结果的最前面（列表：置顶时间倒序 → 创建时间倒序；搜索：置顶优先 → 相似度倒序）。
...
环境要求：**Node.js 22+**、**pnpm 11+**（可用 `corepack enable` 启用）。无需 Cloudflare 账号——D1 在本地 `.wrangler/state` 中运行。

```bash
# 1. 安装依赖
pnpm install

# 2. 本地 Worker 配置：示例文件可直接用于 wrangler dev
copy wrangler.example.jsonc wrangler.jsonc      # macOS/Linux: cp wrangler.example.jsonc wrangler.jsonc

# 3. 本地服务端密钥（任意长随机串；此文件不要提交）
copy .dev.vars.example .dev.vars
#   然后编辑 .dev.vars，生成随机串可用：
#   node -e "console.log(require('crypto').randomBytes(48).toString('hex'))"

# 4. 初始化本地 D1 表结构
pnpm db:migrate:local

# 5. 启动（vite build && wrangler dev）
pnpm dev
```

打开 wrangler 输出的地址，默认 **http://localhost:8787**。

其他脚本：

| 脚本 | 用途 |
| --- | --- |
| `pnpm dev` | 构建前端并启动 `wrangler dev`（静态资产 + API + 本地 D1） |
| `pnpm build` | Vite 生产构建到 `dist/` |
| `pnpm typecheck` | `tsc --noEmit` |
| `pnpm test` | Vitest 单元/集成测试 |
| `pnpm db:migrate:local` | 迁移应用到本地 D1 |
| `pnpm db:create` | 创建远程 D1 数据库 |
| `pnpm db:migrate:remote` | 迁移应用到远程 D1 |
| `pnpm deploy` | 构建 + `wrangler deploy` |
| `pnpm dry-run` | 只校验 Worker 产物，不部署 |

## 部署到 Cloudflare

需要一个免费的 [Cloudflare 账号](https://dash.cloudflare.com/sign-up)。认证方式二选一：本地执行一次 `wrangler login`，或在 [API Tokens](https://dash.cloudflare.com/profile/api-tokens) 创建具备 Workers + D1 权限的令牌并导出 `CLOUDFLARE_API_TOKEN` / `CLOUDFLARE_ACCOUNT_ID`。

```bash
# 1. 创建 D1 数据库——复制输出的 database_id
pnpm db:create

# 2. 把 database_id 填入 wrangler.jsonc
#    （如需自定义域名，可同时取消注释 "routes" 并改成自己的域名）

# 3. 远程数据库执行迁移
pnpm db:migrate:remote

# 4. 设置密封密钥（32+ 字节随机串，仅服务端持有）
...
首次部署后可通过 `https://<worker名>.<你的子域>.workers.dev` 访问；自定义域名在 `wrangler.jsonc` 中注释掉的 `routes` 块配置。

> ⚠️ **已有留言后不要更换 `VEC_SEAL_SECRET`。** 密封对同一密钥是确定性的，更换密钥会让旧密封向量与新搜索向量不兼容（旧留言将搜不到）。轮换密钥需要配合重新密封迁移，或在空库时操作。

### 使用 GitHub Actions 部署

`.github/workflows/deploy.yml` 始终执行类型检查与构建，只有仓库显式开启时才会部署——fork 出去默认安全跳过。在你的仓库 **Settings → Secrets and variables → Actions** 中配置：

- **Variables（变量）**
  - `ENABLE_DEPLOY` = `true`：push 到 main 自动部署的总开关
...
- 登录成功进入「管理模式」：首页每张留言卡（含搜索结果）出现 **删除 / 置顶** 入口；右下角主题按钮旁多出 **修改密码** 浮标（弹窗内完成，无独立页面）；
- 会话：登录返回 Bearer token（D1 `admin_sessions`，7 天过期）；连续输错 5 次锁定该 IP 10 分钟；
- 修改密码需当前密码 + 新密码（4–128 位），成功后撤销其它会话、保留当前会话。

> 安全提示：这是「隐藏后门」式的轻量管理，不面向公网强对抗；部署后请尽快登录修改默认密码。
...
端到端冒烟（需先执行 `pnpm db:migrate:local` 并保持 `pnpm dev` 运行）：
...
前端采用模块化切分：自研单根路由（pageId）+ 页面视图 + 组件级同名 CSS。
...
src/
  ...
  router/                   # 自研轻量单根路由（pageId，URL 保持干净）
  pages/                    # 页面视图（每页同名 CSS）
    HomeView                # 论坛主页：搜索 + 留言流（置顶优先 + 时间排序）；发布用弹窗
