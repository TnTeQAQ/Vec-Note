...
1. **留言**：输入标题 + 内容 → 浏览器对标题做 `embed()` 向量化与 `signTitle()` 签名 → 只上传 `内容 + 公开向量 + 密文`。
2. **存储**：Worker 用 `VEC_SEAL_SECRET` 把公开向量密封成 `sealed_vector` 落库；`title_ct` 是密文；`content` 明文。
3. **搜索**：浏览器对查询词向量化 → Worker 密封后按余弦相似度召回 Top-10 候选（仅返回 `密文 + 内容`，不返回相似度/向量）→ 前端对每条结果用 BLS 验签，通过则显示「✓ 解密成功」，否则「相似候选」。

对照示例（提交标题「手机」、内容「测试内容」）：
- 搜「手机」→ 命中，显示「✓ 解密成功」。
- 搜「手」→ 命中（共享字符，向量重叠），显示「相似候选」。
- 搜「电话」→ 不命中（仅字符 n-gram，无共享字符；这是已接受的取舍）。
...
- 正交变换**精确保持余弦相似度**，「手→手机」命中行为不变；
- 「字符→哈希桶」的位置映射被密钥打乱，无密钥无法从 `sealed_vector` 反推字符；
- API 永不返回向量字段。
- 剩余泄漏：可观察到「词项个数/幅度直方图」（≈ 标题规范化后的词项数量级）。如需隐藏长度痕迹，可注入噪声（默认关闭）。
...
覆盖：向量化的确定性与重叠行为；BLS 签名/验签（含篡改检测）；密封的余弦保持与确定性。
